...
export class ChatbotComponent implements OnInit {
  ...
  constructor(
    ...
    private questionTemplateComponent : QuestionsTemplateComponent,
  ...
  sendMessage() {
    ...
    if (this.formValue.match("question is")) {
      let questionTag = this.formValue.replace("question is", '')[0];
      this.question.questionTag = questionTag;
      console.log(questionTag);
    }
    if (this.formValue.match(",")) {
      let optionString = this.formValue.replace("options are", '')[0];
      ...
      this.surveyService.saveQuestion(this.question).subscribe(
        data => {
          console.log(data);
          this.questionTemplateComponent.getQuestionList(this.surveyService.editSurvey.id);
        },
        error => {
          console.log(error);
        }
